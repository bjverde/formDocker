...
#intall Apache + PHP
RUN apt-get -y install apache2 php8.0 libapache2-mod-php8.0 php8.0-cli php8.0-common php8.0-opcache

#PHP Install CURl
RUN apt-get -y install curl php8.0-curl

#PHP Intall DOM, Json, XML e Zip
RUN apt-get -y install php8.0-dom php8.0-xml php8.0-zip php8.0-gd php8.0-soap php8.0-intl php8.0-xsl
...
RUN echo "\n# Custom settings"                                    >> /etc/php/8.0/apache2/php.ini \
    && echo "error_log = /tmp/php_errors.log"                     >> /etc/php/8.0/apache2/php.ini \
    && echo "memory_limit = 256M"                                 >> /etc/php/8.0/apache2/php.ini \
    && echo "max_execution_time = 120"                            >> /etc/php/8.0/apache2/php.ini \
    && echo "file_uploads = On"                                   >> /etc/php/8.0/apache2/php.ini \
    && echo "post_max_size = 100M"                                >> /etc/php/8.0/apache2/php.ini \
    && echo "upload_max_filesize = 100M"                          >> /etc/php/8.0/apache2/php.ini \
    && echo "session.gc_maxlifetime = 14000"                      >> /etc/php/8.0/apache2/php.ini \
    && echo "display_errors = On"                                 >> /etc/php/8.0/apache2/php.ini \
    && echo "error_reporting = E_ALL & ~E_DEPRECATED & ~E_STRICT" >> /etc/php/8.0/apache2/php.ini

# Set PHP security settings
RUN echo "\n# Security settings"                    >> /etc/php/8.0/apache2/php.ini \
    && echo "session.name = CUSTOMSESSID"           >> /etc/php/8.0/apache2/php.ini \
    && echo "session.use_only_cookies = 1"          >> /etc/php/8.0/apache2/php.ini \
    && echo "session.cookie_httponly = true"        >> /etc/php/8.0/apache2/php.ini \
    && echo "session.use_trans_sid = 0"             >> /etc/php/8.0/apache2/php.ini \
    && echo "session.entropy_file = /dev/urandom"   >> /etc/php/8.0/apache2/php.ini \
    && echo "session.entropy_length = 32"           >> /etc/php/8.0/apache2/php.ini
...
RUN echo "xdebug.remote_log=/var/log/apache2/xdebug.log" >> /etc/php/8.0/mods-available/xdebug.ini
